...
class GameResultScraper(KBOBaseScraper):
    ...
    def _parse(self, response):
        ...
        if not maxInnings:
            logger.warning("No inning datas found in response.")
            return None, None
    ...
    def fetch(self, season, date):
        ...
        for schedules in games.values():
            for schedule in schedules:
                ...
                logger.info(f"Fetching result for game id {game_id}...")
                ...
                try:
                    response = fetch_json(self.url, self.payload)
                    if not response or int(response.get("code", 0)) != 100:
                        logger.warning(f"No valid response for game id {game_id}.")
                        continue

                    headers, rows = self.parse(response)
                    if not rows:
                        logger.info(f"No rows returned for game id {game_id}.")
                        continue
                    ...
                    logger.error(f"Error fetching result for game id {game_id}: {e}")
